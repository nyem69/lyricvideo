...
messages/
  ...
  ms.json
  id.json
src/lib/paraglide/        # GENERATED by paraglide compile / the Vite plugin. COMMITTED (as in nasab/manahalal) so `pnpm check`/CI have it without an extra ordering step.
...
- **Generated `src/lib/paraglide/` and CI ordering** → commit the generated output (as the reference repos do) + `build: "paraglide-js compile && vite build"` + `postinstall: "paraglide-js compile"`, so `pnpm check`/CI never see a missing module.
...
- **D3 — Commit the generated `src/lib/paraglide/`** (match the reference repos) rather than gitignore it.
